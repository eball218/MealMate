...
export default function PlanView({ preferences, mealPlan, onRegenerate, loading, onAddToShoppingList, onToggleLike }: Props) {
  ...
  // Scroll to meal when selectedDateIndex changes (mobile mainly)
  useEffect(() => {
    if (window.innerWidth < 768 && itemRefs.current[selectedDateIndex]) {
                 ...
                 return (
                  <div 
                     ...
                     className={`group bg-white rounded-[24px] overflow-hidden transition-all duration-500 cursor-pointer border ${
                        isExpanded
                          ? 'border-lime-400 shadow-[0_20px_40px_-10px_rgba(163,230,53,0.2)] ring-1 ring-lime-400/50 md:col-span-2 lg:col-span-2 md:row-span-2' 
                          : 'border-transparent shadow-sm hover:shadow-md hover:border-stone-100'
                     }`}
                  >
                     {/* Header Row */}
                     <div className={`p-3 pr-5 h-full ${isExpanded ? 'flex flex-col' : 'flex items-start gap-4'}`}>
                        {/* Image Thumbnail */}
                        <div className={`relative flex-shrink-0 transition-all duration-500 ${isExpanded ? 'w-full h-64 rounded-2xl mb-4' : 'w-20 h-20 rounded-xl'}`}>
                            <img 
                              src={meal.imageUrl} 
                              alt={meal.title} 
                              className={`w-full h-full object-cover ${isExpanded ? 'rounded-2xl' : 'rounded-xl'}`}
                            />
                             {isExpanded && (
